...
- ✅ Broadcast flood — *input goes to every node; a node linked to something in the broadcast re-broadcasts and appends itself, so a route arrives carrying its whole chain of reasoning. `openplexus/broadcast.py`. **The direction, taken on architecture rather than on the numbers** — it matches the incumbent and does not beat it.*
- ✅ Stamina in place of a floor — *a route carries a budget the edges it walks refuel, so one that walked strong edges can afford a weak one where a floor cuts on the step in front of it. That is what stamina buys and a floor cannot express.*
...
- ⬜ Many origins in place of edge kinds — *was ❌ for scoring below chance, which was a front-end artefact: under k-means it is 0.1870 against one origin's 0.1802, three seeds, all above chance. **Still no advantage** — the gap is well inside the spread — but it is no longer harmful.*
...
- ⬜ The broadcast flood as a cross-modal walk — *was ❌ on 150-question runs under the hash. **Re-run at n=1,000 it MATCHES the incumbent**: under k-means, flood 0.1802 against `equivalence_classes` 0.1807, chance 0.108, every seed above it. The null was the front end, not the flood. **It does not BEAT it**, and costs 172,000 expansions a question where the classes walk costs none.*
