...
- ⬜ FB15k-237, floor established before anything is built on it — *the leak that killed its predecessor is genuinely gone: inverse rules mined from train score 0.45 applied to train and 0.0001 applied to test, and no test triple appears verbatim or reversed. What it hands over instead is the marginal — relation-tail frequency alone reaches MRR 0.2334 — and published DistMult and ComplEx sit at 0.241 and 0.247 (TransERR arXiv 2306.14580 Table 3). **So two widely cited results are within 0.014 of a baseline with no structure in it**, and the quantity worth reporting on this benchmark is the margin over the marginal, not the MRR.*
