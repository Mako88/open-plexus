...
- ⬜ An interactive environment, not a recorded corpus — *John's position, 2026-08-01: this kind of model may need to ACT to learn at all. It is the only source that supplies an error signal for free, and every mechanism this project still lacks — prediction, the trigger for asking, an output channel — needs one. A recorded corpus can never say whether acting to disambiguate beats watching.*
