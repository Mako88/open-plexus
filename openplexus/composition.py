...
class Composition:
    ...
    def __init__(self, relations: int) -> None:
        if relations < 1:
            raise ValueError("a composition over no relations has no answers")
    ...
    def surface(self, role: str, relation: int) -> int:
        ...
        if not 0 <= relation < self.relations:
            raise ValueError(
                f"relation {relation} is outside 0..{self.relations - 1}. "
                f"Folding it back in would make two different relations one "
                f"surface, and every count on it would be a mixture")
        return ROLES.index(role) * self.relations + relation
    ...
    def ranked(self, left: int, right: int, statistic: Statistic,
        ...
        **The candidate is the first argument and the half is the second**, which
        ...
        from_left = self.surface("left", left)
        from_right = self.surface("right", right)
        scored = []
        for candidate in range(self.relations):
            answer = self.surface("target", candidate)
            score = rule(statistic(self.index, answer, from_left),
    ...
    def table(self, statistic: Statistic, combine: str = "min",
        ...
        for left in range(self.relations):
            for right in range(self.relations):
                scored = self.ranked(left, right, statistic, combine)
