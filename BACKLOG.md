...
| **BUILD THE DAMAGED-QUERY TASK** | [g10-09](experiments/sweeps/g10-09-is-there-similarity-to-generalise.txt) found the first property that survives measurement: a half-corrupted query still retrieves at **0.930** where a cache returns nothing at all. This is the discriminating task, now specified rather than hoped for, and it is realistic — a node on a lossy link reconstructing keys it does not store is exactly where corruption enters (note 024) | mine |
