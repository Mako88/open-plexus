...
ARM_ORDER = ("baseline", "consolidate", "persist")
...
def main() -> None:
    ...
    if ("baseline", points[0]) in cells and ("baseline", points[-1]) in cells:
        total = movement("baseline", points[0], points[-1])
        print(f"  P1  CONTROL. baseline moves under 0.05 bits from "
              f"{points[0]:,} to {points[-1]:,}: {total:+.4f} -> "
              f"{'CONFIRMED' if abs(total) < 0.05 else 'REFUTED'}")
        print("      Decision 63 measured 0.039 against a 0.04 seed spread. A "
              "baseline that does not reproduce makes nothing else readable.")

    if ("consolidate", points[0]) in cells:
        total = movement("consolidate", points[0], points[-1])
        print(f"  P2  consolidate is also flat: {total:+.4f} -> "
              f"{'CONFIRMED' if abs(total) < 0.05 else 'REFUTED'}")
        print("      Consolidation without persistence is a WITHIN-sequence "
              "mechanism, so more data cannot help it either. This arm is what "
              "separates 'consolidation helps' from 'persistence helps'.")

    if ("persist", 62_500) in cells and ("persist", 125_000) in cells:
        late = movement("persist", 62_500, 125_000)
        base_late = (movement("baseline", 62_500, 125_000)
                     if ("baseline", 62_500) in cells else 0.0)
        breaks = late > SEED_SPREAD
        print(f"  P3  THE GATE. persist keeps improving past the wall -- "
              f"62,500 to 125,000 exceeds the {SEED_SPREAD} seed spread: "
              f"{late:+.4f} (baseline {base_late:+.4f}) -> "
              f"{'CONFIRMED' if breaks else 'REFUTED'}")
    ...
    norms = {c: statistics.mean([r["lasting_norm"]
                                 for r in cells[("persist", c)]])
             for c in points if ("persist", c) in cells}
    if norms:
        print("\n  slow-store norm at the end of training: "
              + ", ".join(f"{c:,}->{v:.2f}" for c, v in norms.items()))
