...
- ⬜ Spending the codes where the data is, without fitting a codebook — *the hash's remaining deficit, about 0.44 of purity in every feature space with structure in it. **Now known to be decisive rather than cosmetic**: at the hash's purity no walk clears chance on the senses graph and at k-means' purity both do. A data-free front end cannot know where the data is, and this is what that costs.*
...
- ❌ ~~Refuel a broadcast route on SURPRISE rather than strength~~ — *swept over a grid that spans its own per-step deficit of 2.11 — the first grid, 0.002–0.2, sat entirely inside the argmax-only regime and varied nothing. Properly swept it stays at 0.100–0.103 against a chance of 0.108.*
- ❌ ~~Fund on strength and VALUE the arrival by rarity~~ — *John's "aha grounded in logic": every surviving route is well-supported, and among them the ones landing somewhere rare score highest. Three seeds at the best budget: **0.1287 / 0.0923 / 0.0989** against strength's 0.1345 / 0.0769 / 0.1099. Both at chance, indistinguishable.* **Revives if** measured on a graph a walk can find anything in.
