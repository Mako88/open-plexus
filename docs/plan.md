...
| A deeper walk for prediction | Monotonically worse — without edge kinds, deeper reaches more and ranks worse. **A carried discount FLATTENS that curve without raising its peak**, so overspending stops costing and the best score does not move | **Edge kinds**, and that refutation reproduced |
...
| `Window` span | Null on snake, WORSE on `Babi` at far more traffic, and **the whole task on `Rhythm`**. **Revival RUN twice**: kinds recover much of it; **discounting a carried edge does not**, moving along the frontier the budget already describes rather than pushing it out — and starving the walk outright below a stamina the plain arm walks at | **Something making the edge worth its ROW, not cheaper to walk past.** Kinds were the structural half; a weight is not the other |
...
- **TWO ARMS CAN PEAK AT DIFFERENT BUDGETS, AND THEN ONE SWEEP COMPARES ONE AT ITS
  BEST AND THE OTHER AT ITS WORST.** Inhibition peaks two settings above the
  one-sided count and was refuted at the count's peak. **Sweep the dial for BOTH
  arms and compare PEAK TO PEAK** — a difference at one setting is not a ranking.
  **Every arm in step 4 is owed that.**
