# Decision log, entries 1–82 — ARCHIVE

> **This is history. Do not read it for the current state of the project.**
> Entries are chronological, superseded entries are **not** marked as such, and
> several were overturned by later work. For what is true now, read
> [STATE.md](../../STATE.md). For the reasoning behind the current line, read
> [DECISIONS.md](../../DECISIONS.md), which holds entries 83 onward.

**What this covers.** The MQAR era, the g8/g9 gating line, the first corpus
benchmark, and the readout work — decisions taken between 2026-07-25 and
2026-07-27. Entry 83 is where the relational/chain line begins, which is the
work that is live now, so that is where the split falls.

**Entries here that are still load-bearing** and get cited from current work:

| entry | what it settled |
|---|---|
| 52 | C1 was a proxy; John replaced it with *"does it work over the internet"* |
| 54 | our rank collapse is the data being low-rank, not the rule failing |
| 63 | the model converges at ~16,000 characters — the data axis saturates |
| 66 | two different "effective ranks" existed here; name the measure beside the number |
| 69 | six mechanisms move the LEVEL, none moves the SLOPE |
| 70 | the readout was the ceiling |
| 78 | four self-imposed limits audited and un-constrained |
| 82 | window 16 is worth 11.97× over lock-step on an 80 ms link losing 2% |

Everything else here is superseded, closed, or about machinery that no longer
carries a live question.
