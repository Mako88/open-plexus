...
- ⬜ Many surfaces per input, with the columns COMMUNICATING — *the arm above refutes independent columns, not columns. Each predicted alone; nothing let one column's surface inform another's, which is the one thing this architecture is otherwise entirely about.*
