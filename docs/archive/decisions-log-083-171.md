# ARCHIVED — the append-only decision log, entries 83–171

**Replaced by [DECISIONS.md](../../DECISIONS.md)**, which is an option tree rather
than a log. 2026-07-29.

**Why it was replaced.** This file reached 6,040 lines. Nothing could read it whole,
so it was read selectively, and on its last day that produced three wrong
recommendations in a row — each built on a claim a later entry had already
superseded. Decision 115 closed saturation and entries 042, 133 and 134 reopened it
in turn. Its own Index stopped being maintained at entry 134, and entry 132 has an
Index row and no body.

John's diagnosis, which is the right one: *"the intention of decisions was not to be
an append-only log. The intention is to make sure we don't circle back to things that
have already been proven. But for that to be at all useful, it has to be a short
bulleted list."* **A log records; it does not prevent.**

**This file is kept, not deleted, and it is still the reference.** Every attempt in
the tree cites its entry number here, so the reasoning behind any decision is one
lookup away. What changed is which document is authoritative: **the tree is, and this
is the footnotes.**

**Read it by looking an entry up, never forward from a point.** Entries were never
rewritten when later work overturned them, so reading sequentially will hand you
claims that were true when written and are false now. Four mechanisms here were
built, measured and abandoned; two headline numbers were retracted.

Entries 1–82 are in [decisions-001-082.md](decisions-001-082.md).
