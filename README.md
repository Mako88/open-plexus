...
- ⬜ Spending the codes where the data is, without fitting a codebook — *the hash's remaining deficit, about 0.44 of purity. **Decisive rather than cosmetic**: at the hash's purity no walk clears chance on the senses graph and at k-means' purity both do.*
- ⬜ SEVERAL coarse hashes per item instead of one fine one — *the first legal repair for that deficit. Four families of 6 bits: each alone is 62 codes at q 0.3635 with 63 items behind every code, and their conjunction identifies at **0.9845** — but the conjunction is never built, so no code is starved. Data-free, so two nodes still agree exactly.*
