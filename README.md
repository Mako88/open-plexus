...
- ⬜ Many seeds in place of edge kinds — *the typed walk discriminated by route kind, which a senses graph has not got and whose questions have not got either. The broadcast's answer is that several surfaces fire together and their routes converge. **An occasion currently supplies about four** — `spectra` returns one row per recording and one image is one code, so a claim of hundreds was wrong. Untested at four.*
